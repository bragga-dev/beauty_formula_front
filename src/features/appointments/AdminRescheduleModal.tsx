...
// Mesma janela de dias do fluxo padrão de agendamento (BookingPage) e do
// reagendamento do cliente — mantém o mesmo teto calculado no backend.
...
/**
 * Reagendamento do admin — mesmo fluxo do cliente (`RescheduleAppointmentModal`):
 * reaproveita `useEligibleEmployees` + `useCalendarAvailability`, então um
 * horário só aparece aqui se o backend confirmou que existe vaga real.
 * A diferença é só o endpoint chamado no submit (`onSubmit` vem do
 * `useAdminSchedulingMutations().update`, não do `reschedule` do cliente)
 * e o campo de observações, que o admin pode ajustar junto.
 */
export function AdminRescheduleModal({ open, scheduling, isSubmitting, onSubmit, onClose }: AdminRescheduleModalProps) {
  ...
  // Ao abrir, pré-seleciona o profissional atual e as observações atuais.
  useEffect(() => {
  ...
  return (
    <Modal open={open} onClose={handleClose} title="Reagendar atendimento" size="lg">
      <p className="text-sm text-bone-400">
        Escolha um novo horário para <span className="text-bone-100">{scheduling.service.name}</span>. Só
        aparecem horários com vaga real confirmada pelo backend — o mesmo cálculo usado no agendamento do
        cliente.
      </p>
      ...
      <div className="mt-6 flex justify-end gap-3 border-t border-ink-700 pt-5">
        ...
        <Button type="button" disabled={!selectedSlot} isLoading={isSubmitting} onClick={handleConfirm}>
